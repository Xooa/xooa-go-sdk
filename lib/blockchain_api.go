...
/* GetTransactionByTransactionId Get Transaction data (block #)
 Get specific Transaction information such as hash ,Read_set Write Set
 * @param ctx context.Context for logging, tracing, etc.
...
func (a *BlockchainApiService) GetTransactionByTransactionId(ctx context.Context, txHash string, localVarOptionals map[string]interface{}) (models.Transaction, models.PendingTransactionResponse, *models.XooaApiException) {
	var (
		localVarHttpMethod = strings.ToUpper("Get")
		localVarPostBody interface{}
		localVarFileName string
	...
	// create path and map variables
	localVarPath := BasePath + "/transactions/{TxHash}"
	...
	if err := typeCheckParameter(localVarOptionals["async"], "string", "async"); err != nil {
		log.Error(err)
		return transaction, pendingTransactionResponse, &models.XooaApiException{
			ErrorCode: 0,
			ErrorMessage: err.Error(),
		}
	}
	...
	localVarHttpResponse, err := a.client.callAPI(r)
	if err != nil || localVarHttpResponse == nil {
		return transaction, pendingTransactionResponse,  models.NewXooaApiException(localVarHttpResponse)
	}
	...
	if localVarHttpResponse.StatusCode == 202 {
		log.Warn(localVarHttpResponse.StatusCode,localVarHttpResponse)
		json.Unmarshal(content, &pendingTransactionResponse)
		return transaction, pendingTransactionResponse, nil
	}
	if err != nil {
		log.Error(err)
		return transaction, pendingTransactionResponse, &models.XooaApiException{
	...
	if async == true {
		...
		json.Unmarshal(content, &transaction)
	}
	log.Info(transaction)
	return transaction, pendingTransactionResponse, nil
}

/* GetTransactionByTransactionIdAsync Get Transaction data async (block #)
...
func (a *BlockchainApiService) GetTransactionByTransactionIdAsync(ctx context.Context, txHash string, localVarOptionals map[string]interface{}) (models.Transaction, models.PendingTransactionResponse, *models.XooaApiException) {
	localVarOptionals["async"] = "true";
	return a.GetTransactionByTransactionId(ctx,txHash, localVarOptionals);
